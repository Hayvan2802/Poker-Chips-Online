...
describe('Update safeguards',()=>{
  it('compares public, npm, and legacy versions by the same release number',()=>{
    ...
    expect(isNewerVersion('0.11.0','0.0.10')).toBe(true)
  ...
  it('shows only the latest note on first install and migrates a legacy seen-version',()=>{
    expect(releasesSince(releases,null).map(x=>x.version)).toEqual(['0.11'])
    expect(releasesSince(releases,'0.0.5').map(x=>x.version)).toEqual(['0.11','0.10','0.9','0.8','0.7','0.6'])
